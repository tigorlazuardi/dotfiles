...
2. Each slice's worker + reviewer return `knowledgeDelta[]` (new conventions discovered while building/reviewing).
...
Default rule: `writeDirectly = slice.lowTolerance === true`. Low-tolerance slices get a frontier-reviewer gate at review time, so mid-slice writes are safe and propagate faster. Routine slices batch.
