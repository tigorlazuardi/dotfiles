...
const xml = String.raw // for syntax highlight
...
export default function(pi: ExtensionAPI) {
    pi.on("before_agent_start", (event, ctx) => {
        ...
        const context = xml`<runtime-model-context provider="${escapeXml(provider)}" model="${escapeXml(id)}" capability="${capability(id)}" name="${escapeXml(name)}"/>`;
        return {
