# rate-limit-wakeup — manual set + multi-scope TUI (SPEC draft)

Status: FASE 1 (spec/design). No code yet.
...
## Implementation environment (Opsi A — work in chezmoi source)

The extension has TWO locations. Ralph hats MUST follow this:

```txt
EDIT (source of truth, git repo):
...
No push inside the loop. No `chezmoi apply` skipped — source and runtime must never drift.
