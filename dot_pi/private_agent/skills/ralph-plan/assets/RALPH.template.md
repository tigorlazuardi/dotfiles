<!--
  RALPH.template.md — Ralph Loop contract.
  ...
  alone: assume the orchestrator remembers nothing except this text and what's in the repo + RALPH_PROGRESS.md.
-->
...
This contract is authored for an orchestrator running as **`<orchestrator-model>`** (Opus chose this when planning; it is also recorded in RALPH_PROGRESS.md). Before reading tasks or spawning anything, confirm your own model matches.
...
`RALPH_PROGRESS.md` at the start of every iteration and act on whatever is incomplete.
...
subagent you spawn to report caveman too. Stay normal for: code, commit messages, `RALPH_PROGRESS.md` content,
...
except as the final line of a true terminal state. Everywhere else (notes, plans, RALPH_PROGRESS.md, talking
...
record it in RALPH_PROGRESS.md and treat it as an abort condition.
...
`RALPH_PROGRESS.md` is the source of truth for progress. On every iteration: read it, find the first
incomplete batch, and continue. After **each** task reaches a terminal status, update RALPH_PROGRESS.md and
...
**Resume reconciliation.** A crash can leave a worktree with uncommitted WIP that RALPH_PROGRESS.md doesn't
...
RALPH_PROGRESS.md. Don't restart a task that's actually half-done on disk.

**Heartbeat.** Each iteration, append one line to RALPH_PROGRESS.md's progress log: `iteration N · <what
advanced>`. Lets a human `tail` progress and powers no-progress detection below.

## No-progress guard (stuck = stop, don't burn tokens)

Track `last-progress-iteration` in RALPH_PROGRESS.md — the last iteration a task reached `done`. If the current
...
- **The reviewer self-verifies.** The `reviewer` agent re-runs commands and re-reads code to verify its own findings before reporting — it does not spawn a nested verifier. Front-loading a scout `MAP` into the review prompt is an optional optimization for an especially expensive Opus review; skip it otherwise.

## Per-task execution loop

For each task in the current batch:

1. **Implement.** Spawn `implementer` with `model: sonnet`. Give it: the task, its acceptance
...
2. **Read the implementer's `RESULT`:**
   - **`HANDOVER`** — the implementer hit its context budget and wrapped up cleanly. This is **not** a
     ...
     immediately spawn a **fresh** `implementer` (`model: sonnet`) for the same task, passing
...
4. **Review.** S/M tasks: spawn `reviewer` with `model: sonnet`, give it the diff + acceptance
   criteria. L tasks: **don't review one-by-one** — let the batch's L tasks accumulate, then send them
   to a **single** `model: opus` `reviewer` spawn following "Opus economy" above (full diffs,
...
5. **Outcome.**
   - **Pass** (verification green AND reviewer `PASS`): merge the worktree into the integration
     branch (if parallel), mark the task `done`, commit, update RALPH_PROGRESS.md.
...
When a task fails twice, do not keep grinding. Spawn `reviewer` with `model: opus` as a
...
- **ABORT:** record the decision and reason in RALPH_PROGRESS.md, write the abort into the summary, clean up
...
5. Write the **merge-handoff summary** into RALPH_PROGRESS.md: what changed, how it was verified, any
