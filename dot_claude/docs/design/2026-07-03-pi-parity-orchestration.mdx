...
<Callout type="warn" title="Partially superseded 2026-07-07">
The plandeck renderer/skill decisions in this doc are superseded by the Astro Docs Standard
(`plans/astro-docs-standard/SPEC.mdx`): Plandeck retired, `.mdx` dialect now Astro/Starlight
per `astro-docs-authoring` + `astro-docs-setup`. Orchestration decisions remain in force.
</Callout>
...
preflight, the usage guard (now the global `usage-limit-wakeup` hook, superseding the retired
`usage-checkpoint` skill — 2026-07-13), trunk-based mode, FLEET-RESUME-HOOK.
</Decision>
...
- **usage guard**: originally the `usage-checkpoint` skill invoked on every Build entry; since
  2026-07-13 this is automatic via the global `usage-limit-wakeup` hook (arms a post-reset
  `CronCreate` wakeup when usage runs high — no manual invocation). Timer-before-100% invariant.
...
| 11 | usage-checkpoint wording: wave barrier → scheduling-loop checkpoints _(skill later retired 2026-07-13, replaced by the `usage-limit-wakeup` hook)_ | `skills/usage-checkpoint/SKILL.md` (removed) | XS | 4 |
