...
name: ralph-plan
description: >-
  Use this to author a Ralph Loop contract — the PROMPT.md plus resume state — that a Sonnet
  orchestrator (Sonnet by default, Opus for a low-correctness-tolerance slice) will execute via /ralph-loop:ralph-loop in a git worktree. Trigger this whenever the
...
# Ralph Plan — author a contract for `/ralph-loop:ralph-loop`
...
the Ralph Loop plugin. You (Opus) are the planner; you do **not** run the loop. You interview the
...
> ⛔ `/ralph-plan` needs Opus. You're on a different model. Switch to an Opus model (e.g. `/model` →
...
The orchestrator runs resident across the whole loop — every iteration, for hours — so its model is the biggest cost multiplier in the run. Choose it deliberately and **write it into the contract**: the `<orchestrator-model>` placeholder in PROMPT.md and an `orchestrator_model:` line in STATE.md. Users routinely forget to set the session model, so the contract also makes the loop **early-exit on mismatch** (the first section of the PROMPT template).
...
- **`PROMPT.md`** — the contract the orchestrator re-reads every iteration. Build it from
  `assets/PROMPT.template.md`. It must be fully self-contained, because `/ralph-loop` feeds its entire
  text back verbatim each loop and the orchestrator may have no other memory.
- **`STATE.md`** — the resume ledger. Build it from `assets/STATE.template.md`. The orchestrator reads
...
Fold the **Sanity Check** and **Implementation Strategy** directly into `PROMPT.md` (the template has
sections for them). They are not separate files: the orchestrator only ever re-reads `PROMPT.md`, so
anything it must act on lives there.

## Step 4 — Subagents

Execution uses four committed subagents — `ralph-implementer`, `ralph-reviewer`, `ralph-scout`, and `ralph-verifier`
(shipped alongside this skill under `~/.claude/agents/`). The orchestrator picks the model **per
...
in its frontmatter (all `sonnet`), but the per-spawn `model` parameter **overrides** it, so the Opus
routing for L tasks holds. You do not generate per-model agent files; routing is logic in `PROMPT.md`.
Verify all four agents exist; if missing, tell the user to (re)install them.

**The reviewer scouts and verifies itself (nested subagents).** `ralph-reviewer` carries the `Agent` tool, so it spawns its own helpers and you do **not** pre-scout for it:
- When an L review needs context beyond the diff (callers, related flows, all uses of a symbol), the reviewer spawns `ralph-scout` (`model: haiku`; bump to `sonnet` for a hard multi-hop trace) itself and reads only the returned `path:line` map. That fan-out reading stays inside the reviewer's context, never yours.
- Before it rejects a finding on an L / risky task, the reviewer spawns `ralph-verifier` (one per finding) to adversarially try to refute it; a finding that survives is real. This keeps reject precision high without you mediating.

Only `ralph-reviewer` nests; `ralph-scout`, `ralph-verifier`, and `ralph-implementer` are leaves (no `Agent`), so the tree stops at depth 2. Front-loading a scout map into a review prompt is now an optional optimization for a very expensive Opus review, not a requirement. Drop the old "pre-scout for the reviewer" rule from any contract you author — it is obsolete.

**Caveman by default.** The contract makes the orchestrator and every spawned subagent
(`ralph-implementer`, `ralph-reviewer`) report caveman-compressed — drop articles/filler/hedging, keep
full technical substance — because narration is the cheapest token to cut and subagent reports flow
back into the orchestrator's context. Code, commit messages, `STATE.md`, the `<promise>` tag, and
security points stay normal. This is already baked into the templates and agent files.

**Implementers don't compact — they hand over.** A `ralph-implementer` that nears its context limit
...
A hole in the contract is paid for many times over in the loop. Re-read the finished `PROMPT.md` and
`STATE.md` with fresh eyes and fix inline:
...
2) Switch model to Sonnet.
3) /clear  (or open a fresh session) — you'll run as the orchestrator.
4) Paste:

/ralph-loop:ralph-loop "$(cat plans/<scope>/001-<slice-name>/PROMPT.md)" --max-iterations <N> --completion-promise "RALPH SLICE FINISHED"
```
...
committed, OR an abort reason recorded in STATE.md," so it's truthfully emittable on both success and
