...
- `frontend-design` (distinctive aesthetics, anti-generic) governs VISUAL CHARACTER; this skill governs STRUCTURE, hierarchy, restraint. Not in conflict: distinctive ≠ cluttered — apply boldness to few emphasized elements, restraint to everything else. If they pull opposite ways on a product UI, scannability/simplicity wins; on marketing/landing pages, distinctiveness gets more rope.
- Spacing details → ui-spacing. Depth/shadows → ui-depth. Colors/tokens → ui-color-theming. Conversion flows → ux-psychology.
- **Framework stance (user preference, overrides the video)**: DO use established UI systems/frameworks (shadcn/ui, Radix, Tailwind, etc.) — they cover a11y and edge cases hand-rolled CSS misses. Design-system principles here apply ON TOP of a framework (token discipline, hierarchy), never as a reason to hand-roll components.
