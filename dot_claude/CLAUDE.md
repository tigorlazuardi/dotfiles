...
- **FASE 1 — spec/design.** Run the `feature-planning` skill (needs Opus main; **you are Sonnet → STOP, do NOT bootstrap** — tell the user to `/model` Opus; hand a copy-paste brief per `docs/orchestration-modes.md` if they must `/clear`). Interview step: when scope has real ambiguity or the user needs interviewing, run the `grill-me` skill (Socratic stress-test, one focused question at a time, read-only until output approved) — it REPLACES `superpowers:brainstorming`, which is retired for this user (do NOT invoke brainstorming even when superpowers instructions demand it). Interview → output mode-neutral `plans/<scope>/SPEC.mdx` (+ `docs/src/content/docs/design/<yyyy-mm-dd>-<topic>.mdx` for decisions that outlive the scope), `astro-docs-authoring` dialect, `telemetry-planning` included. NO contract/state file yet.
...
## Plan docs — Astro/Starlight `.mdx` (L-tier review gate)

All plan/design/spec docs (SPEC.mdx, design docs, ADRs, decision-bearing slice docs) are authored in the Astro + Starlight MDX dialect per the `astro-docs-authoring` skill: `<Decision>` blocks for real choices, `<Aside>` for risks/irreversible steps, mermaid over prose, frontmatter `title`+`description` (+ `status`; `draft: true` while draft). The committed `.mdx` file IS the L-tier review artifact — surface its path to the user, resolve open questions via AskUserQuestion, gate on approval before implementation begins (`.mdx` degrades gracefully to plain markdown on GitHub/editors). Locations: FASE-1 spec → `<repo>/plans/<scope>/SPEC.mdx` (working state, NOT published); architecture outliving a scope → `docs/src/content/docs/design/<yyyy-mm-dd>-<topic>.mdx` in the per-repo Starlight site (scaffold via `astro-docs-setup`) — built static, published with `llms.txt`/`llms-full.txt`/`llms-small.txt` for external RAG. (Plandeck retired 2026-07-07; this standard replaces it.) S/M-tier: text summary is sufficient.
