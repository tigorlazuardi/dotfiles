...
- Feature plan / SCOPE.md / ADR / RALPH.md / PROMPT.md drafting.
...
5. **Committed project rule** — which `.claude/rules/<name>.md` codifies the above. If no rule yet, mark as follow-up before merge.
...
- Path-scoped (instrumentation code lives under specific dirs) → `/promote-rules` → writes `.claude/rules/<name>.md` with `paths:` frontmatter.
- Intent-triggered (e.g. "add a metric", "add a span", "instrument this") → `/promote-skills` → writes `.claude/skills/<name>/SKILL.md`.
