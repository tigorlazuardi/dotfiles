...
Ask via AskUserQuestion: **GitHub Pages** vs **Cloudflare Pages** vs **GitLab Pages** (vs skip CI for now). Default to the host matching the git remote — GitLab remote → GitLab Pages. Also ask, if the repo is private, whether the published site/`llms.txt` may be public — private-repo RAG access is a per-repo call (on GitLab this is enforced by Pages access control, a project setting).
